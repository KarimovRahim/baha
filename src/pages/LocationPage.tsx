...
import { MapPin, Sun, Moon, Sparkles, Store, Clock, Navigation } from 'lucide-react';
        ...
        <div className='lg:hidden w-full h-full flex flex-col px-4 py-3 overflow-y-auto'>
          ...
          <div className='flex-1 flex flex-col justify-center'>
            <div className='space-y-3 pb-6'>
              {branches.map((branch, index) => <LocationCard key={branch.id} branch={branch} index={index} isVisible={isVisible} onOpenMap={handleOpenMap} />)}
            </div>
          </div>
        </div>
          ...
          <div className='w-2/3 flex items-center pl-8'>
            <div className={`w-full transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
              <div className='grid grid-cols-1 gap-5'>
                {branches.map((branch, index) => <DesktopLocationCard key={branch.id} branch={branch} index={index} onOpenMap={handleOpenMap} />)}
              </div>
            </div>
          </div>
        </div>
  ...
  return <div className={`relative w-full rounded-xl overflow-hidden transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{
    ...
    willChange: 'transform'
      ...
      <div className='p-5'>
        ...
        <p className='text-sm mb-3 opacity-80' style={{
        color: 'var(--text-secondary)'
      }}>
          {branch.description}
        </p>
        ...
        <div className='flex gap-2'>
          <button onClick={() => onOpenMap(branch.mapUrl)} className='flex-1 py-3 rounded-xl font-semibold text-white transition-all hover:scale-[1.02] flex items-center justify-center gap-2' style={{
          background: 'var(--accent-gold)'
        }}>
            <Navigation size={16} />{t("str_110")}</button>
        </div>
      </div>
    </div>;
};
const DesktopLocationCard = ({
  branch,
  index,
  onOpenMap
}: any) => {
    ...
    willChange: 'transform'
      ...
      <div className='p-6'>
        ...
        <div className='flex gap-3'>
          <button onClick={() => onOpenMap(branch.mapUrl)} className='flex-1 py-4 rounded-xl font-bold text-white transition-all hover:scale-[1.02] flex items-center justify-center gap-2 text-lg' style={{
          background: 'var(--accent-gold)'
        }}>
            <Navigation size={20} />{t("str_111")}</button>
        </div>
      </div>
    </div>;
};
